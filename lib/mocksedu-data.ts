...
export type Course = {
  ...
  export type ContentBlock = {
    id: string;
    type: 'text' | 'video' | 'image' | 'quiz' | 'simulation';
    content: string;
    order: number;
  };
  ...
  export const mockApi = {
    ...
    // New API functions for trading platform features
    ...
    executeVirtualTrade: async (
      ...
      if (success) {
        ...
        return {
          ...
          message: `${side.toUpperCase()} order executed successfully. Portfolio ${portfolioChange >= 0 ? 'gained' : 'lost'} ${Math.abs(portfolioChange).toFixed(2)} USDT.`
        };
      } else {
        return {
          success: false,
