...
import { Search, Edit, Trash2, Eye, Upload, EyeOff, Plus } from 'lucide-react';
...
export const TradeListComponent = ({
    editTrade, filterStrategy, handleAddTrade, isAddTradeOpen, isEditTradeOpen, mistakes, newTrade, searchQuery, 
    setFilterStrategy, setIsAddTradeOpen, setIsEditTradeOpen, setNewTrade, setSearchQuery, setShowHidden, 
    showHidden, strategies, filteredTrades, sortBy, setSortBy, setSortOrder, sortOrder, recentlyEdited, 
    setEditTrade, selectedTrade, handleHideTrade, setSelectedTrade, handleEditTrade, handleDeleteTrade 
}: TradeListProps) => (
    <div className="space-y-4 px-2 sm:px-4 lg:px-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
            ...
            <AlertDialog open={isAddTradeOpen} onOpenChange={setIsAddTradeOpen}>
                ...
                <AlertDialogContent className="max-w-[95vw] sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                    <AlertDialogHeader>
            ...
            <CardContent className="px-2 sm:px-4 pb-4">
                ...
                <div className="space-y-3">
                    {filteredTrades.map(trade => (
                        <div
                            key={trade.id}
                            className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 sm:p-4 border rounded-lg transition-all ${recentlyEdited.includes(trade.id) ? 'bg-green-50 border-green-200' : 'hover:bg-gray-50'}`}
                        >
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
                                <div className="text-left min-w-[80px]">
                                    <div className="font-medium text-sm sm:text-base">{trade.symbol}</div>
                                    <div className="text-xs sm:text-sm text-gray-600">{formatDate(trade.entryDate)}</div>
                                </div>
                                <Badge variant={trade.side === 'Long' ? 'default' : 'secondary'} className="w-fit">
                                    {trade.side}
                                </Badge>
                                ...
                                <div className={`font-medium text-sm sm:text-base ${trade.pnl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                            ...
                            <div className="flex items-center gap-2 mt-3 sm:mt-0 w-full sm:w-auto justify-end sm:justify-start">
                                <AlertDialog>
                                    ...
                                    <AlertDialogContent className="max-w-[95vw] sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
                                        <AlertDialogHeader>
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                                        {trade.screenshots.map((screenshot, index) => (
                                                            <div key={`${trade.id}-screenshot-${index}`} className="border rounded-lg p-4 text-center bg-gray-50">
