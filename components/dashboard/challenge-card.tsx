		...
		<div className="border border-gray-200 dark:border-gray-800 rounded-lg bg-white dark:bg-gray-950 p-4 sm:p-6 hover:shadow-lg transition-shadow">
			...
			<div className="mt-4 sm:mt-6 flex flex-col xs:flex-row xs:items-center xs:justify-between gap-3 xs:gap-4">
				...
				<div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
					...
					<span className="text-xs sm:text-sm font-medium truncate">{startDate}</span>
				</div>

				{/* Reward */}
				<div className="flex items-center gap-1.5 text-amber-600 dark:text-amber-400">
					<Trophy className="h-3.5 w-3.5 sm:h-4 sm:w-4 flex-shrink-0" />
					<span className="text-xs sm:text-sm font-medium truncate">{reward}</span>
				</div>
			</div>
			...
			<div className="mt-3 sm:mt-4">
				<div className="flex justify-between items-center">
					...
					>
						{status === "active" ? "🔥 Active" : status === "upcoming" ? "⏳ Coming Soon" : "✅ Completed"}
					</span>
				</div>
			</div>
		</div>
	...
	const challenges = [
		{
			...
			description: "Complete daily coding exercises to improve your programming skills and build consistency in your learning journey.",
			icon: Code,
		...
		},
		{
			...
			description: "Work with other learners on a collaborative project to practice version control, code reviews, and agile methodologies.",
			icon: Users,
		...
		},
		{
			...
			description: "Solve complex algorithmic problems and compete with peers in this intensive problem-solving competition.",
			icon: Target,
